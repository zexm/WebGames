...
(function() {
	...
	function ga(a, b, c, d) {
		...
		if (e) {
			...
			if (f < 0) {
				...
				b[f] = c;
				a.setProperty("-webkit-transition-duration", b.join(", "), "");
	...
	var I = [], sa = document.getElementById("hydra-stage");
	function ta(a) {
		if (a != m) {
			m && m.z();
	...
	function V(a) {
		...
		a = this.element;
		if (a.className)
			(" " + a.className + " ").indexOf(" button ") >= 0 || (a.className += " button");
		else
			a.className = "button";
		...
		if (m == this.g)
			switch(a.type) {
				case "touchstart":
					var b = this.element;
					if (b.className)
						(" " + b.className + " ").indexOf(" button-down ") >= 0 || (b.className += " button-down");
					else
						b.className = "button-down";
					break;
				case "touchend":
				case "touchcancel":
					if (!a.targetTouches.length) {
						b = this.element;
						var c = b.className.split(" "), d = c.indexOf("button-down");
						if (d >= 0) {
							c.splice(d, 1);
							b.className = c.join(" ")
						}
	...
	function Ja(a, b) {
		for (var c = 0, d = String(a).replace(/^[\s\xa0]+|[\s\xa0]+$/g, "").split("."), e = String(b).replace(/^[\s\xa0]+|[\s\xa0]+$/g, "").split("."), f = Math.max(d.length, e.length), i = 0; c == 0 && i < f; i++) {
			var u = d[i] || "", L = e[i] || "", M = RegExp("(\\d*)(\\D*)", "g"), ba = RegExp("(\\d*)(\\D*)", "g");
			do {
				var N = M.exec(u) || ["", "", ""], ca = ba.exec(L) || ["", "", ""];
				if (N[0].length == 0 && ca[0].length == 0)
					break;
				c = Ka(N[1].length == 0 ? 0 : parseInt(N[1], 10), ca[1].length == 0 ? 0 : parseInt(ca[1], 10)) || Ka(N[2].length == 0, ca[2].length == 0) || Ka(N[2], ca[2])
			} while(c==0)
		}
		return c
	}
	...
	Ra && (ab["9"] || (ab["9"] = Ja(Ua, "9") >= 0));
	function bb(a) {
		return '<div class="label-gameover"><div>Score: <span class="stat">' + Da(String(a.f)) + '</span></div><div>Level: <span class="stat">' + Da(String(a.h)) + "</span></div><div>Judges say... " + (a.f < 10 ? '<span class="rating0">Awful</span>' : a.f < 1E3 ? '<span class="rating1">Weak</span>' : a.f < 5E3 ? '<span class="rating2">OK</span>' : a.f < 12E3 ? '<span class="rating3">Good</span>' : a.f < 2E4 ? '<span class="rating4">Super</span>' : a.f < 3E4 ? '<span class="rating5">Marvellous</span>' : '<span class="rating6">Blockstar</span>') + "</div>" + (a.pa ? '<div class="label-highscore">It\'s a new high score!</div>' : "") + '</div><div class="button-replay button-big">Replay</div><div class="button-quit button-big">Main Menu</div>'
	}
	...
	function cb(a) {
		...
		if (c > 0)
			...
			b += " <div>No scores recorded yet!</div>";
	...
	var Y = [new X(1, [[-1, -1], [-1, 0], [0, 0], [0, -1]]), new X(6, [[-2, 0], [-1, 0], [0, 0], [1, 0]]), new X(2, [[0, -1], [1, -1], [0, 0], [-1, 0]]), new X(0, [[0, -1], [-1, -1], [0, 0], [1, 0]]), new X(4, [[-1, -1], [-1, 0], [0, 0], [1, 0]]), new X(3, [[1, -1], [-1, 0], [0, 0], [1, 0]]), new X(5, [[0, -1], [-1, 0], [0, 0], [1, 0]])];
	...
	function hb(a, b, c) {
		for (var d = a.b.x + b, e = a.b.y + c, f = 0; f < a.b.coords.length; ++f) {
			var i = a.b.coords[f];
			if (d + i[0] < 0 || d + i[0] >= 10 || e + i[1] < 0 || e + i[1] >= 20 || a.k[(e + i[1]) * 10 + (d + i[0])])
				return false
		}
		a.b.x = d;
		a.b.y = e;
	...
	function ib(a, b) {
		for (var c = 0; c < b.coords.length; ++c) {
			var d = b.coords[c];
			if (b.x + d[0] < 0 || b.x + d[0] >= 10 || b.y + d[1] < 0 || b.y + d[1] >= 20 || a.k[(b.y + d[1]) * 10 + (b.x + d[0])])
				return false
		}
		return true
	}
	...
	h.ta = function() {
		...
		this.i.c(new E(0, 500, 0.5, "linear", p,
		void 0));
	...
	h.wa = function(a) {
		...
		a.rotation = -90;
		v(a);
		this.i.c(new na(90, 0.1, "linear", p,
		void 0))
	};
	h.va = function(a) {
		for (var b = 0; b < a.coords.length; ++b) {
			var c = a.coords[b], d = new t(kb(a.color)), e = a.x + c[0];
			c = a.y + c[1];
			this.k[c * 10 + e] = d;
			x(d, e * Z, c * Z);
			A(this, d)
	...
	h.xa = function(a) {
		...
		for (var d = [], e = 0, f = a.length - 1; e < f; ++e) {
			var i = a[e], u = new t(r("explosion"));
			x(u, 0, i * Z);
			A(c, u);
			for (var L = 0; L < 10; ++L)
				( u = this.k[i * 10 + L]) && q(b, u);
			i = i - 1;
			if (i > a[e + 1]) {
				L = Math.sqrt(e + 1) / 4;
				do {
		...
		this.c(new U([G("opacity", "0.8", 0.5, c), new P(function() {
			for (var ba = 0, N = b.length; ba < N; ++ba)
				b[ba].j();
			c.j()
		}), new R(d)]))
	};
	function nb(a, b) {
		...
		if (c < 36E5)
			return Math.round(c / 6E4) + " minutes ago";
		if (c < 54E5)
			return "one hour ago";
		if (c < 864E5)
			return Math.round(c / 36E5) + " hours ago";
		if (c < 1728E5)
			return "yesterday";
		if (c < 2592E6)
			return Math.round(c / 864E5) + " days ago";
		if (c < 3888E6)
			return "one month ago";
		if (c < 31104E6)
			return Math.round(c / 2592E6) + " months ago";
		if (c < 62208E6)
			return "one year ago";
	...
	pb.prototype.load = function() {
		var a = s('<div class="credits">by b_garcia <img src="static/twitter.png"></div><div class="ui-attract"></div>');
		this.d.element.appendChild(a);
		var b = new V(a.querySelector(".credits"));
		b.n = function() {
			window.top.location = "http://twitter.com/b_garcia"
		};
		z(this, b, null);
		var c = new y;
		z(this, c, a.querySelector(".ui-attract"));
		c.c(new S(new U([new P(function() {
			...
			e = o() * 4 + 1;
			ka(d, 2 / e);
			d.c(new U([new E(0, -480, e, "linear", p,
			void 0), new T]));
			A(c, d)
		}), new Q(0.1)])));
		qb(this, false)
	...
	function qb(a, b) {
		var c = new y(s('<div class="mainmenu"><div class="button-play button-big">Play</div><div class="button-scores button-big">Scores</div><div class="button-more button-big">More Games</div></div>'));
		z(a, c);
	...
	ub.prototype.load = function() {
		var a = s('<div class="label-pause">PAUSED</div><div class="button-resume button-big">Continue</div><div class="button-quit button-big">Main Menu</div>');
		this.d.element.appendChild(a);
		x(this.d, -320, 100);
		this.d.element.style.setProperty("opacity", "0", "");
		this.c(H(0, 100, 0.2));
		this.c(G("opacity", "1", 0.2));
		var b = this, c = new V(a.querySelector(".button-resume"));
		c.n = function() {
	...
	h = rb.prototype;
	h.load = function() {
		var a = s('<div class="ui-board"></div><div class="ui-preview"></div><div class="ui-score"></div><div class="ui-level"></div><div class="button-pause"></div>');
		...
		z(this, this.h, null);
		this.d.element.appendChild(a);
		var b = new V(a.querySelector(".button-pause"));
		b.n = function() {
			K(new ub)
		};
		z(this, b, null);
		...
		this.M = new vb;
		w(this.M, 40);
		"lastGame" in n || wb(this.M, yb ? "&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;Welcome to Block Dream!&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;DRAG your finger to move.&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;TAP to rotate.&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;FLICK down to drop!" : "&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;Welcome to Block Dream!&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;Use the arrow keys to move.&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;UP key to rotate.&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;DOWN key to drop!");
		z(this, this.M, a);
		z(this, new lb, a);
	...
	h.ya = function() {
		...
		if ($.f > 0) {
			ka(this.f, 2);
			this.f.c(new F(1, 1, 1, "ease-in", ea,
			void 0))
		}
	};
	h.sa = function() {
		this.da.Y = 1E3 / $.h;
		this.h.element.textContent = "LVL " + $.h;
		if ($.h > 1) {
			wb(this.M, "Level " + Da(String($.h)));
			ka(this.h, 2);
			this.h.c(new F(1, 1, 1, "ease-in", ea,
			void 0))
		}
	};
	h.ra = function(a) {
	...
	h.za = function() {
		if (m == this) {
			if (this.X && (this.W - this.fa) / (this.P - this.X) > 0.25)
				jb($);
			else
				this.I == 1 && $.rotate();
			this.P = this.I = 0
	...
	var yb = "ontouchstart" in window, zb = null, Ab = 0;
	function Bb(a) {
		var b = "";
		switch(a.type) {
			case "mousedown":
	...
	var Fb = new O("intro"), Gb = new t(r("darkness"));
	Gb.c(new U([new ma("opacity", "0", 1, "ease-in",
	void 0), new P(J)]));
	z(Fb, Gb);
	K(Fb);
	function Hb() {
